...
- `시작 전` 상태에서는 일정 상세 모달 우측 하단에 플로트 `여행 시작하기` 버튼을 표시한다.
- `여행 시작하기`를 누르면 `trip_plans.status`를 `in_progress`로 변경하고 시작 버튼을 숨긴다.
- `시작 전` 상태에서는 여행지 상세의 `여행 완료` 버튼을 숨기고, `여행 진행중` 상태부터 표시한다.
- DAY 루트 모달에서 여행지 상세 카드를 펼친 뒤 `여행 완료` 버튼으로 방문 완료 표시를 할 수 있다.
- 모든 여행지를 완료하면 `trip_plans.status`를 `completed`로 변경하고 `completed_at`을 기록한다.
