...
- 상단 인삿말 영역은 표시하지 않고 여행 계획 생성 버튼은 화면 우측 하단에 고정한다.
...
로그인 사용자의 여행 MBTI 점수 순으로 국내 관광지 전체를 탐색하고,
지역과 키워드 조건으로 원하는 여행지를 좁혀 확인한다.
...
- 지역 또는 키워드 선택 → 조건에 맞는 첫 페이지 재조회
- 필터 초기화 클릭 → 전체 관광지 첫 페이지 재조회
- 페이지 버튼 클릭 → 해당 페이지 조회
