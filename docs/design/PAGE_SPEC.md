...
- 브라우저 기본 `alert`, `confirm`은 사용하지 않는다.
- 입력 검증, 저장 성공, API 오류는 공통 Packing 안내 모달로 표시한다.
- 삭제, 데이터 초기화, 회원 탈퇴 등 복구 불가능한 작업은
  확인·취소 버튼이 있는 위험 확인 모달을 사용한다.
- 공통 안내 모달은 `public/scripts/ui-feedback.js`와
  `public/styles/ui-feedback.css`에서 관리한다.
...
필터 옵션은 서비스가 지원하는 17개 광역시·도와 규칙 기반 키워드를
사용하며, 옵션 표시를 위해 관광지 전체 데이터를 조회하지 않는다.
...
- Kakao
- Naver (추후)
- 소셜 로그인 완료 후 동의 이력이 없으면 필수 약관 동의 모달을 표시한다.
...
- 약관 옆 `보기` 버튼 클릭 시 이용약관 또는 개인정보 처리방침 본문을 모달로 표시한다.
- 마케팅 수신 동의 항목은 제공하지 않는다.
...
- 메인 허브 페이지로 이동
- 구글·카카오 로그인 완료 → 필수 약관 동의 확인 후 최초 1회만 프로필 설정 모달 표시
- 소셜 프로필 설정 → 닉네임 필수, 프로필 이미지 선택 후 저장해야 서비스 진입
...
- 소셜 로그인은 Supabase OAuth를 사용하며, 로그인 완료 후 필수 약관 동의 이력이 없으면 동의 모달을 표시한다.
...
- 개인정보처리방침
- 문의 링크
- 설정의 이용약관, 개인정보처리방침은 버튼 형태로 표시하고 클릭 시 본문 모달을 표시한다.
...
- 내 여행 클릭 → 전체 여행 목록 이동
- 설정 클릭 → 계정 설정 페이지 이동
- 설정의 이용약관/개인정보처리방침 보기 클릭 → 정책 본문 모달 표시
...
- 선택 키워드는 일정 생성 요청의 `keywords` 배열로 전달되며, 일치 키워드가 많은 관광지를 후보 상단에 배치한다.
