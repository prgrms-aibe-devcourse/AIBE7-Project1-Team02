...
`importTourData.js`는 서비스에서 지원하는 광역시·도별로 TourAPI를
조회한다. `TOUR_API_REGIONS`에 쉼표로 구분한 지역명을 지정하면 해당
지역만 수집하며, 미설정 시 17개 지역 전체를 수집한다. 주소, 이미지,
좌표 또는 상세 설명이 없는 관광지는 저장하지 않는다. 각 지역은
관광지 40개, 문화시설 25개, 음식점 25개, 레포츠 10개를 목표로 하며,
유형별 목표를 채우면 남은 페이지와 상세 설명 요청을 중단한다.
